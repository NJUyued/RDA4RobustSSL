...
def split_ssl_data(dataset, data, target, num_labels, num_classes, save_path, index=None, include_lb_to_ulb=True, 
                         noisy='none', noisy_ratio=0, 
                         mismatch='none',
                         n0=0,
                         gamma=0):
    """
    ...
    Args
        noisy: type of noise
        noisy_ratio: the ratio of noise to labeled data
        mismatch: type of mismatched distribution
        n0: for imbalanced labeled data
        gamma: for imbalanced unlabeled data
...
def sample_labeled_data(data, target, 
    ...
    if noisy_ratio==0:
        ...
        assert noisy_ratio >= 0 and noisy_ratio <= 100
        assert noisy != 'none'
        num_noisy = int((noisy_ratio/100.0) * samples_per_class)
        print('-'*100)
        print(str(int(num_noisy))+' noise per class')
        print('-'*100)
